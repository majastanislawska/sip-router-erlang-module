...
int fixup_cmd_erlang_cast(void** param, int param_no){
	...
	if (param_no == 3) {
		...
		s.len = strlen(s.s);
		if(s.len==0) {
			LM_ERR("cmd_erlang_cast: param %d is empty string!\n", param_no);
			return -1;
		}
		if(pv_parse_format(&s ,&model) || model==NULL) {
			LM_ERR("cmd_erlang_cast: wrong format [%s] for value param!\n", s.s);
			return -1;
		}
		*param = (void*)model;
		return 0;
...
int cmd_erlang_cast(struct sip_msg* msg, char *cn, char *rp, char *ar) {
#define AVP_PRINTBUF_SIZE 1024
	...
	if(msg==NULL) {
	    LM_ERR("cmd_erlang_cast: received null msg\n");
	    return -1;
	}
	...
	for(node=nodes_lst;node;node=node->next) {
		LM_DBG("cmd_erlang_cast: matching %s with %.*s\n",node->name,conname.len,conname.s);
	...
	//User passed term will be sedond element of tuple, first is gen_cast atom
	// no pid in this message 
	ei_x_encode_tuple_header(&argbuf, 2);
	ei_x_encode_atom(&argbuf, "$gen_cast");
